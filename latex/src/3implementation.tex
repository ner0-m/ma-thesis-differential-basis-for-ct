...
thesis for personal reasons. \todo{maybe delete personal reason maybe not!}
...
A classical ray-driven approach is proposed in
...
projection. However, they do suffer from certain artefacts. This is especially true for Siddon's
method, due to the interpolation of the Joseph's method less artefacts appear there. These kind of
artefacts can be seen in the experiments section of this thesis (\autoref{chap:experiments}).
...
pair~\cite{zeng_unmatched_2000}. If the system matrix \(A\) is constructed using one projector, it
the backward projector should constructed \(A^T\), i.e.\ the transpose of the matrix. If the
abstract view of linear bouded operators is taken, this would be the aforementioned adjoint.
...
\inlinetodo{Mention usage of cubic B-Spline and parameters for blobs}
