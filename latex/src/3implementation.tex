...
I want to dedicate a chapter to the C++ framework \textit{elsa}~\cite{lasser_elsa_2019} developed at
the \gls{CIIP} research group. As the practical part of my thesis is implemented for elsa, I feel
...
should be close the either mathematical notation and mathematical concepts. Second, it should be
useful for the research done at the \gls{CIIP} research group. And finally, it should be as easy as
possible for other to reproduce results from the research. These different aspects have to be
wagered against each other, as they might stand in conflict to each other.
...
suite (though, we can still improve there), a merge request based workflow, which heavily relies on
...
These includes concepts for residuals, functionals (such as \(l_1\), \(l_2\), or the Huber norm) and
...
As the experiments are run using elsa. Hence, some necessary details which hold for all the
experiment setups given later on. The overall geometric setup elsa follows is as described in
...
\begin{definition}[Structural Similarity Index Measure]
	...
	\]
	where \(\mu_{\mvec{x}}\) is the average of \(\mvec{x}\), \(\mu_{\hat{\mvec{x}}}\) is the
	average of \(\hat{\mvec{x}}\), \(\sigma_{\mvec{x}}^2\) is the variance of \(\mvec{x}\),
...
B-Spline based projector in \autoref{fig:sino_diff_blob_bspline}, once can see clear differences in
...
behaviour can be seen. As already mentioned, \autoref{fig:sinogram_difference_blob_bspline_crop}
...
rectangle. The rectangle is centered in the image and the has a constant value of \(1\) inside the
...
\begin{figure}
	...
	\caption{Overview of the different phantoms used for the synthetic tests. From left to
		...
		abdomen. All phantoms are are displayed with a windowed to values in the range \([0,
...
\(180^\circ\) projection angle is excluded. The one-dimensional detector has a resolution of
...
based metrics (i.e.\ \gls{MSE}, \gls{NRMSE} and \gls{PSNR}). However, the visual perception is
backed up by the \gls{SSIM}. The Siddon's method performs quite a bit worse (around \(8\%\)
compared to both the blob and B-Spline one), and both the blob and B-Spline based projector can
outperform the Joseph's projector. Though against the later one, it is a small difference.
...
interval is a touch, but notably higher for both the blob and B-Spline basis functions. For such a
complex phantom, the difference seem to vanish.
...
projection is performs specifically bad at angles around each \(90^\circ\). Furthermore, it poorly
handles noise. It should be noted however, that the Siddon's method performs equally well or
...
quality in the reconstruction of the human abdomen. This behaviour does not reflect the visual
inspection. Further, both methods perform well in the presence of added noise to the forward
projection.
...
this section, both basis function have a support of \(2\) units.
...
possible background tasks are closed during the benchmarks and the machines is left running until
...
the three-dimensional case are plotted. Noticeably, the method's scale similarly as both the
Siddon's and Joseph's method, but the performance drop can be seen there as well.
...
\begin{figure}
	...
	\caption{Runtime in seconds for the 4 different projection methods, both for the forward
		...
		the line plot show the tolerance interval based on the standard deviation of the
...
\begin{figure}
	...
	\caption{Runtime seconds for the 4 different projection methods, both for the
		...
		shaded area around the line plot show the tolerance interval based on the standard
...
and B-Spline based projectors performance quite consistent compared to the Siddon's and Joseph's,
...
\begin{figure}
	...
	\caption{Violin plot for the forward projection of a selection of the sizes, individually
		...
		line, the thicker line is the inter-quantile range, the thin bars give the give the
...
a behaviour, were two cases are hit, in one, the projector finishes quickly, on the other it takes a
