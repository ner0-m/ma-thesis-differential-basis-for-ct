...
Generally usual notation regarding symbols such as the natural numbers \(\mathbb{N}\), integers
...
characters such as \(x \in \mathbb{R}^n\). Elements of \(n\)-dimensional vectors are subscribed
\(x_j\). Row vectors are specifically denoted with brackets such as \([x_1, x_2, x_3, \dots ]\).

Given \(\mvec{x} \in \mathbb{R}\) and \(\mvec{y} \in \mathbb{R}\), then \(\abs{\mvec{x}}\) is the
componentwise absolute value, \(\norm{\mvec{x}}\) is no further specified norm,
...
\(n\)-sphere defined as \(\mathscr{S} = {\mvec{x} \in \mathbb{R} \colon \norm{\mvec{x} = 1} }\).
\(\theta^\perp\) denotes a vector perpendicular to \(\theta\). Though at one place, the scalar
product is also denoted by \(\mvec{x} \cdot \mvec{y}\). \(\mvec{x}^{(k)}\) is the \(k\) vector of a
sequence of vectors. Given the matrix \(A \in \mathbb{R}^{n \times m}\). Then \(A^T\) is the
...
Other important symbols include \(\mu\), the attenuation coefficient, \(\Psi\), the phase-shift of a
...
\(g \coloneq \radon f\) is the Radon Transform of \(f\). Given two functions \(h\) and \(\delta\),
\(h \ast \delta\) is the convolution of the two functions. \(\nabla f\) is the gradient of the
function \(f\).
...
the matter and once it passed through it will hit some kind of detector. This can either measure the
...
without opening it. As touched on in the introduction (\ref{chap:introduction}), this was used to
find and plan the removal of metal objects in bodies.
...
The objective of tomographic reconstruction for X-ray attenuation, is the retrieval of the
...
The graph on the right of each sub-image, shows the measured projections values. The higher this
...
parallel to each other, are precisely the above mentioned projections. This is what is exemplarily
...
noted, that the line integral of a function is the basis of many imaging modalities.

Stepping back, to again a more practical approach. CT scanners evolved over time. Hence, there do
exists many so-called generations of CT scanners. The first and mostly interesting ones because a
lot of the theory is build around their principles, are CT scanners, with a parallel beam geometry
(see \autoref{fig:parallel_beam_geometry}). There all rays are generated in parallel to each other
and hit the detector. This is achieved by having an X-ray source and the detector shift
...
modalities. Specifically, emphasize is put on the similarities in the forward model regarding X-ray
attenuation CT\@.
...
grating, creates an intensity modulation of the detector read-out. This is referred to as
...
Other names for signal are common, such as \textit{image}. However, image is typically an overloaded
...
A note for completeness's sake, recall \autoref{def:inverse_problem} of the inverse problem. There
the definition is given in terms of Hilbert spaces. For our purposes \(\mathcal{H} = \mathbb{R}^I\),
\(\mathcal{K} = \mathbb{R}^J\) and \(A \in \mathbb{R}^{I\times J}\) with \(I \in \mathbb{N}\) and
\(J \in \mathbb{N}\) two numbers. Now, the forward model can be defined.
...
\begin{definition}[Radon Transform]\label{def:radon_transform}
	...
	to sufficiently nice. Then the mapping \(\radon f\colon (\mathbb{R}^n \to \mathbb{R})
...
\mathbb{R}\), one can define the hyperplane \(\mathcal{H}^{n-1}(\theta, s) = \lbrace \mvec{x} \in
\mathbb{R}^n \, \colon \langle \mvec{x}, \theta \rangle = s  \rbrace\) with distance \(s\) to the origin
...
\begin{equation}\label{eq:radon_transform}
	\radon f(\theta, s) = \radon_\theta f(s) = \int_{\langle \mvec{x}, \theta \rangle = s} f(\mvec{x}) \, \, \mathrm{d}\mvec{x}
\end{equation}
...
Transform \(\xray\) was developed \citeauthor*{solmon_x-ray_1976}~\cite{solmon_x-ray_1976}, which is
very similar to the Radon Transform, but integrates over lines for all dimensions, instead of
hyperplanes. See \autoref{sec:xray_transform} for more on the X-ray Transform.

Also note that many notations are common. Specifically in two dimensions, it is common to define it
via the polar coordinates. Others define a subspace perpendicular to the projection direction
\(\theta^\perp\). But the basic principle is the same.
...
The backprojection is basically a smearing of the projection angles along. This is illustrated in
\autoref{fig:backprojection_shepp_logan} on the left-hand side. For more and more back-projections,
the image gets closer to the original phantom. However, stays a bully version of the original
phantom. A sharper reconstruction can be achieved through different means, this is discussed in
...
discussions and nice illustrative figures. More methods to compute the adjoint and reconstruction
