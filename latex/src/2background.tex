...
and then the next acquisitions is performed and so on. It is common to describe a projection with an
angle in two dimensions and two angles in three dimensions. This fully qualifies a single projection
pose. A further notice, it is of no importance for the reconstruction, if the object of interest is
rotated or the source and detector are rotated. X-ray sources for common CT scanners are vacuum
tubes, which convert electrical power to radiation. The detector consists of an array of pixels,
which reacts sensitive to X-rays. For a detailed overview see
...
(for now, this is kept in the two-dimensions). Then the measured intensity is given by the
...
backward projection. The least squares problem also already solves on the issues arising from the
