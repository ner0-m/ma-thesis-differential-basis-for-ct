...
Equations, which are not part of a definition are specifically marked with a number. However,
equations which are part of the definition do not have a specific numbering scheme.
...
A note for completeness's sake, recall \autoref{def:inverse_problem} of the inverse problem. For our
...
\mathbb{s}\), one can define the hyperplane \(\mathcal{H}^{n-1}(\theta, s) = \lbrace \mvec{x} \in
...
\inlinetodo{Insert Result of blurred non-filtered back projection version}
...
\inlinetodo{Use something like to show back-projection and filtered back projection
