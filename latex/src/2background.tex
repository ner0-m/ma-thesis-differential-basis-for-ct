...
deeper dive into the mathematical formulation of the forward model for X-ray attenuation CT\@.

\begin{definition}[Signal]\label{def:signal}
	Let \(f\colon \mathbb{R}^n \to \mathbb{R}\) be a \(n\)-dimensional continuous function,
	whose support is bounded. It is referred to as a \(n\)-dimensional signal. And often it will
	be referred to as signal, without the special mention of \(n\)-dimensional.
\end{definition}
...
the solution domain of the inverse problem, as e.g.\ sinograms are stacks of projections. Hence,
sinograms are not referred to as images. A similar distinction is made for the three-dimensional
case. There the three-dimensional signal is referred to as \textit{volume}.
...
different applications. Usually, some restrictions are put upon the function \(f\), such that it has
...
transformation mapping a function into all its line integrals is known as the Radon Transform, which
is attributed to Johann Radon and his work in
...
Transform \(\xray\) was developed by \textcite{solmon_x-ray_1976}, which is very similar to the
Radon Transform, but integrates over lines for all dimensions, instead of hyperplanes. See
\autoref{sec:xray_transform} for more on the X-ray Transform.
...
direction \(\mvec{\theta}^\perp\). But the basic principle is the same. A notation used throughout
the thesis, is the subscript of the projection angle \(\theta\) as in \(\radon_\theta\). This is
only relevant for two-dimensional Radon Transform.

The Radon Transform is related to the Fourier Transformation. Also, it is a special case of the
Hankel Transform. A special case of the Radon Transform is the \textit{Abel Transform}. If the
...
The back-projection is basically a smearing of the projection angles along all projection
directions. This is illustrated in \autoref{fig:backprojection_shepp_logan} on the left-hand side.
For more and more back-projections, the image gets closer to the original signal. However, it
continuous to be a blurred version of the original.
...
the adjoint operation of the Radon Transform. However, these results are very theoretical. A more
...
of the projected data measured at the angle \(\theta\), is the same as a line going through the
origin of the \(2\)-dimensional Fourier Transform of the complete signal. Hence, with enough
projections, \(f\) can be described fully in the Fourier domain and reconstructed using the inverse
...
\begin{definition}[Permissible representation]\label{def:permissible_representation}
	...
	given by
	\[ f \approx \near{f} = \sum_{k=1}^{N} c_k \varphi_k(\mvec{x}) \]
...
functions introduced by \textcite{lewitt_multidimensional_1990}; and an interesting
...
\begin{figure}
	...
	\caption{Visualization of the voxel basis function. \subref{fig:pixel_basis_function_1d}
		...
		\subref{fig:pixel_basis_approx_fn} shows an approximation of a sine curve using the
...
key component of the forward model, the voxel basis function needs certain special treatment for
such models. Frequently, some form of analytical differentiation is performed, such as
...
First introduced by \textcite{lewitt_multidimensional_1990} in
\citeyear{lewitt_multidimensional_1990}, spherically-symmetric volume elements (often referred to as
...
(SPECT)~\cite{wang_3d_2004, yendiki_comparison_2004}, X-ray attenuation
...
Regarding, the other parameters, \(a\) and \(\alpha\), \textcite{matej_practical_1996} already
conducted investigations regarding good choices of \(\alpha\) and \(a\) for Cartesian grids
including an optimization method. A key take-away is that the exact choice depends highly on the
concrete problem. However, they propose a \textit{standard blob} with parameters with \(a = 2\) and
\(\alpha = 10.4\). The exact choice of \(\alpha\) is in this case is motivated by the aim to find a
band-limited signal representation. The rationale for this is laid out
in~\cite{lewitt_multidimensional_1990} and also discussed in~\cite{benkarroum_blob_2015}.
...
portable across systems. As only two out of three major implementations of the standard library ship
the special mathematical functions. Explicitly, it is not implemented in libc++.

However, for our cases it is sufficient to assume \(m \in \mathbb{N}\). Then
\autoref{eq:radon_blob_basis} can be simplified. A short walk-through of the necessary steps for the
mentioned simplification are outlined. The recurrence formulation for the modified Kaiser-Bessel
function of the first kind is (c.f.~\cite[Chapter~9]{abramowitz_handbook_1972}):
\begin{equation}\label{eq:kaiser_bessel_recurrence}
	I_{m+1}(x) = I_{m-1}(x) - \frac{2 m}{x}I_m(x)
\end{equation}
Further, the Kaiser-Bessel functions of half-integer order have representations with elementary
functions. For the modified Kaiser-Bessel function of the first kind, there are defined as
(c.f.~\cite[Chapter~10]{abramowitz_handbook_1972}):
...
the modified Kaiser-Bessel function of the first kind. The steps are exemplary shown for \(m = 0\).
To keep everything a little more concise, let \(w = \sqrt{1 - \left(\frac{r}{a}\right)^2}\), then:
...
specifically, \textcite*{unser_fast_1991} show the continuous image representation using B-Splines.
...
\end{definition}
Throughout the thesis, only univariate B-Splines are considered. Much research and fields use
non-univariate B-Splines. This can create confusion and make it hard to find relevant topics. For
all mentions of B-Splines, only the univariate B-Spline is referred to.
...
blob basis function. Even though as mentioned above, the B-Splines tend to spherically-symmetric
functions, many such
...
\textcite{horbelt_discretization_2002}. Given the projection angle \(\theta\), then it is:
\begin{equation}
...
referred to as spline bi-kernels. \citeauthor{horbelt_discretization_2002} present an explicit
formulation to compute them.
...
\textcite{entezari_box_2012} proof that the X-ray transform of a \(d\)-variate box spline is a \(d -
1\) variate box spline. Further, it is the box spline defined by the projection of the direction
matrix \(\Xi\). This means, going back to B-Splines, that for any dimension, the X-ray transform of
B-Splines are again B-Splines of lower dimension.
...
blurry version of the original function. \textcite{buzug_computed_2008} describes post-processing as
a possible solution. However, there exists another option. If the projections are filtered in the
Fourier domain and then the filtered projections are back-projected just as before, a sharper image
can be obtained. This is referred to as the \gls{FBP}~\cite{ramachandran_three-dimensional_1971}.
...
\begin{definition}[Filtered Back-Projection]\label{def:filtered_back_projection}
	...
	\autoref{def:back_projection}, then \(\radon^\ast g^\delta\) is the
	\textit{\glsentryfull{FBP}}.
\end{definition}
...
Using the notation given in \autoref{def:forward-model}, i.e. Let \(J\) be a set of scalar
measurements, and \(m_j \in \mathbb{R}\) with \(j \in \lbrace 1, \dots, J \rbrace\) a single scalar
measurement. Then the scalar measurement can be approximated using the permissible representation
...
For a single measurement \(m_j\), the contributions can be gathered in a single vector \(\mvec{a}_j
...
visualization the sparsity of the contribution vectors.
...
should be noted that it is equivalent to solve for \(\mvec{c}\) or \(f\). As from now on the
discretized version is assumed, the one seeks to solve for the coefficient vector \(\mvec{c}\).
...
\begin{definition}[Least Squares Problem]\label{def:least_squares_problem}
...
reconstruction is presented. These sections should provide an overview, of what methods exist and a
brief introduction on how they work. Out of scope is a detailed discussion about convergence of
...
The \glsentryfull{ART} was proposed by \textcite{gordon_algebraic_1970}. However, outside of
tomography it is often known as \textit{Kaczmarz method}~\cite{kaczmarz_approximate_1993}. Though,
\gls{ART} is a slight modification of the original Kaczmarz method.
...
hyperplanes will not intersect at a single point, but rather in close proximity to each other.
...
`simultaneously' is part of the name of \gls{SIRT}.

\section{Regularization}\label{sec:regularization}

So far all algorithms presented in this chapter, have only looked at the least squares problem.
...
prediction models the noisy data. However, we do not want to fit the noise in the data. Therefore,
the additional regularization term incorporates the prior knowledge or assumption. \(\lambda\)
controls the importance or the balance of the two terms.

\subsection{Tikhonov Regularization}\label{subsec:tikhonov_regularization}

A well studied and often used regularization method in the tomographic space is named after Andrey
Nikolayevich Tikhonov~\cite{tihonov_solution_1963}. The penalty restricts the solution based on the
Euclidean norm.
...
\(l_2\)-regularization. Other common choices are the first and second derivative
operator~\cite{golub_tikhonov_1999}. The hope is, that Tikhonov regularization suppresses
