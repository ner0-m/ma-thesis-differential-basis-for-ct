...
def readedf(filename):
    ...
    if nrOfDimensions == 4:
        data = data.reshape(header_dict['Dim_1'],header_dict['Dim_2'],header_dict['Dim_3'],header_dict['Dim_4'])
        
    if nrOfDimensions == 2:
        data = data.reshape(header_dict['Dim_1'],header_dict['Dim_2'])

    if nrOfDimensions == 3:
        data = data.reshape(header_dict['Dim_1'],header_dict['Dim_2'],header_dict['Dim_3'])
...
def writeedf(data, filename):
    ...
    header['Date'] = date.today().strftime('%d-%b-%Y')
...
def imshow3dfull(img, fig=None):
    ...
    img_artist = ax.imshow(data[:,:,int(shape[2]/2)],interpolation='none',cmap='gray',vmin=v_min,vmax=v_max)
...
def imshow(img,fig=None,show_colorbar=True):
    ...
    if fig is None:
        ...
        ax = plt.axes()
        # ax = fig.axes[0]
    img_artist = ax.imshow(img,interpolation='none',cmap='gray',vmin=v_min,vmax=v_max)
    if show_colorbar:
        fig.colorbar(img_artist)
    fig.canvas.draw()
    plt.show()

sino = readedf("3dsinogram_Joseph.edf")
vol = readedf("3dreconstruction_Joseph.edf")
imshow3dfull(vol)
